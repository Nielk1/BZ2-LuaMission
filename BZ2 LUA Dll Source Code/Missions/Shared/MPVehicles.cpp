...
namespace MPVehicleManager
{
	...
	char MPVehicleList[MAX_MP_VEHICLES][MAX_ODF_LENGTH];
	...
	void Init(void)
	{
		...
		//char TempStr[128] = {0};
		for(int i = 0; i < MAX_MP_VEHICLES; i++)
		{
			//sprintf_s(TempStr,"network.mpvehicles.entry%d",i);
			//const char *Contents = GetVarItemStr(TempStr);
			const char *Contents = GetNetworkListItem(NETLIST_MPVehicles, i); // Test me!
			if(Contents != NULL)
				strcpy_s(MPVehicleList[i], Contents);
			/*
			{
				strncpy_s(MPVehicleList[i], Contents, MAX_ODF_LENGTH);
			}
			*/
		}
	...
	// Given a bitfield (hopefully only 1 bit on) saying which vehicle is present, return the odf for that slot
